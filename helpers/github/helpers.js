...
const _getIssueCreated = async (request, data = {}) => {
  ...
  if (!response.ok()) {
    throw new Error(
      `Failed to create issue: ${response.status()} ${await response.text()}`
    );
  }
...
const _getIssueData = async (request, issueNumber) => {
  ...
  if (!response.ok()) {
    throw new Error(
      `Failed to get issue: ${response.status()} ${await response.text()}`
    );
  }
...
const _getIssueComments = async (request, issueNumber) => {
  ...
  if (!response.ok()) {
    throw new Error(
      `Failed to get issue comments: ${response.status()} ${await response.text()}`
    );
  }
...
const _closeIssue = async (request, issueNumber) => {
  ...
  if (!response.ok()) {
    throw new Error(
      `Failed to close issue: ${response.status()} ${await response.text()}`
    );
  }
